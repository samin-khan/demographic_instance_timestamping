...
def detect_face(image_paths,  SAVE_DETECTED_AT, default_max_size=800,size = 300, padding = 0.25):
    cnn_face_detector = dlib.cnn_face_detection_model_v1(dir_path + 'dlib_models/mmod_human_face_detector.dat')
    sp = dlib.shape_predictor(dir_path + 'dlib_models/shape_predictor_5_face_landmarks.dat')
    base = 2000  # largest width and height
    for index, image_path in enumerate(image_paths):
        print("{}: {}".format(index, image_path))
        if index % 1000 == 0:
            print('---%d/%d---' %(index, len(image_paths)))
        img = dlib.load_rgb_image(dir_path + image_path)
...
def predidct_age_gender_race(save_prediction_at, imgs_path = 'cropped_faces/'):
    ...
    model_fair_7.load_state_dict(torch.load(dir_path + 'dlib_models/res34_fair_align_multi_7_20190809.pt'))
    model_fair_7 = model_fair_7.to(device)
    model_fair_7.eval()

    model_fair_4 = torchvision.models.resnet34(pretrained=True)
    model_fair_4.fc = nn.Linear(model_fair_4.fc.in_features, 18)
    model_fair_4.load_state_dict(torch.load(dir_path + 'dlib_models/res34_fair_align_multi_4_20190809.pt'))
...
if __name__ == "__main__":
    ...
    if "tv" in args.input_csv:
        predidct_age_gender_race("test_tv_outputs.csv", SAVE_DETECTED_AT)
    else:
